...
export default function AdminPanel() {
  ...
  const handleEmergencyAction = async (action: string) => {
    if (action === 'shutdown') {
      const secret = prompt('Enter OWNER_SHARED_SECRET:');
      if (!secret) return;
      // Owner override handled server-side
    }
    ...
    try {
      const res = await fetch('/api/admin/emergency', {
        ...
        body: JSON.stringify({ action, secret: action === 'shutdown' ? secret : undefined }),
      });
      if (res.ok) {
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="p-4 border rounded">
          ...
          <p className="mb-2">Current: <strong>{siteState}</strong></p>
        ...
        <div className="p-4 border rounded">
          ...
          <ul className="space-y-2">
            <li><a href="/admin/verifications" className="text-primary hover:underline">Review Verifications</a></li>
            <li><a href="/admin/copyright" className="text-primary hover:underline">Copyright Claims</a></li>
            <li><a href="/admin/bans" className="text-primary hover:underline">Manage Bans</a></li>
          </ul>
        </div>
      </div>
            ...
            <tbody>
              {logs.slice(0, 10).map((log) => (
                <tr key={log._id}>
                  ...
                  <td className="border p-2">{log.targetType} ({String(log.targetId).slice(0, 6)}...)</td>
